...
# --- 웹 페이지 설정 ---
st.set_page_config(page_title="소비기한 시뮬레이터 v3", layout="centered")

st.title("🍳 맞춤형 식품 소비기한 시뮬레이터 (v3)")
st.write(
...
if "육류" in food_type:
    ...
    # [논문 반영] 계란의 초기 일반 세균 수 약 5.0 * 10^3 CFU/ml 반영
    N0 = 5 * (10**3)
    base_aw = 0.96
elif "유제품" in food_type:
    # [논문 반영] 초기 가공 유제품 세균 수는 낮지만 영양분이 많아 증식이 빠름
    N0 = 10**2
    base_aw = 0.97
...
# --- 세균 성장 속도(mu) 계산 (온도 및 식품 특성 맞춤형 물리 공식) ---
if temp <= 0:
    ...
    # [논문 기반 피드백 반영 디테일 설계]
    if "계란" in food_type:
        # 계란은 껍질(큐티클층) 장벽이 있어 온도가 아주 높지 않으면(25도 이하) 증식이 매우 느림
        if temp < 25:
            mu = mu * 0.3
        else:
            mu = mu * 1.2  # 고온 노출 시 큐티클층 붕괴로 급격히 침투 및 증식
    elif "유제품" in food_type:
        # 유제품은 미생물뿐만 아니라 산도 변화 및 지방 산화 속도가 고온에서 기하급수적으로 빨라짐
        if temp > 25:
            mu = mu * 1.5
...
ax.plot(time_steps, bacterial_counts, label=f"{food_type} Growth Curve", color="#e74c3c", lw=2.5)
ax.axhline(
